...
Entrez.email = "dkiselev@u.rochester.edu"
...
snp = get_snp([rsID])
print('\n')
possibleIssues = snp[rsID]['LOC']
print("Possible association(s) to rsID#",rsID)
print("----------------------------------------------")
for i in possibleIssues:
	handle = Entrez.efetch(db="gene", id=i['geneId'], rettype='xml',retmode='xml')
	response = handle.read()
	for data in response.splitlines():
		if data.__contains__("Gene-ref_desc"):
			print("Possible association(s):", i['symbol'],"|",cleanTag(data).strip(' '))
